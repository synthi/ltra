// lib/Engine_Ltra.sc | v0.9
// LTRA Audio Engine - GOLDEN MASTER
// Full Matrix Implementation (Explicit Routing)
...
Engine_Ltra : CroneEngine {
    ...
    *new { arg context, doneCallback; ^super.new(context, doneCallback); }
    ...
    alloc {
        // Bus de audio interno para alimentar a Softcut
        bus_looper_in = Bus.audio(context.server, 2);

        SynthDef(\ltra_core, {
            arg out, looper_bus,
                // --- OSCILLATORS (Base Parameters) ---
            ...
            // Smoothing de Controles (Anti-Zipper)
            var lag = 0.05;
            var s_freq1 = Lag.kr(freq1, lag); var s_freq2 = Lag.kr(freq2, lag);
            var s_freq3 = Lag.kr(freq3, lag); var s_freq4 = Lag.kr(freq4, lag);
            var s_vol1 = Lag.kr(vol1, lag); var s_vol2 = Lag.kr(vol2, lag);
            var s_vol3 = Lag.kr(vol3, lag); var s_vol4 = Lag.kr(vol4, lag);
            var s_filt1 = Lag.kr(filt1_tone, lag); var s_filt2 = Lag.kr(filt2_tone, lag);
            var s_dtime = Lag.kr(delay_time, 0.2); 

            // --- 2. GENERACIÓN DE MODULADORES ---
            ...
            // Outline: Internal Gates vs External Audio
            ...
            // --- 3. CÁLCULO DE MATRIZ (EXPANDIDO) ---
            // Nota: Usamos NamedControl.kr para acceder a los ~80 parámetros de la matriz
            // sin saturar la lista de argumentos de la función principal.
            
            // Helper para limpieza visual, pero expandido lógicamente
            ...
            m_amp1   = calc_mod.("amp1", arp_cv1);
            m_shape1 = calc_mod.("shape1", arp_cv1);

            // VOZ 2
            m_pitch2 = calc_mod.("pitch2", arp_cv2);
            m_amp2   = calc_mod.("amp2", arp_cv2);
            m_shape2 = calc_mod.("shape2", arp_cv2);

            // VOZ 3
            m_pitch3 = calc_mod.("pitch3", arp_cv3);
            m_amp3   = calc_mod.("amp3", arp_cv3);
            m_shape3 = calc_mod.("shape3", arp_cv3);

            // VOZ 4
            m_pitch4 = calc_mod.("pitch4", arp_cv4);
            m_amp4   = calc_mod.("amp4", arp_cv4);
            m_shape4 = calc_mod.("shape4", arp_cv4);

            // EFECTOS GLOBALES (Usamos arp_cv1 como fuente genérica de arp si se rutea)
            m_filt1   = calc_mod.("filt1", arp_cv1);
            m_filt2   = calc_mod.("filt2", arp_cv1);
            m_delay_t = calc_mod.("delay_t", arp_cv1);
            m_delay_f = calc_mod.("delay_f", arp_cv1);

            // --- 4. OSCILADORES (Morph Noise->Saw FM) ---
            mk_osc = { |f, s| 
                var noise = PinkNoise.ar;
                // FM de Ruido sobre Sierra: Característica clave del Alt
            ...
            // Generación de Voces con Modulación Aplicada
            ...
            o4 = mk_osc.(s_freq4 * (2.pow(m_pitch4)), (shape4 + (m_shape4*4)).clip(0,4)) * (s_vol4 + m_amp4).clip(0,1) * mk_vactrol.(gate4, t_arp4);

            sig_mix = Pan2.ar(o1, pan1) + Pan2.ar(o2, pan2) + Pan2.ar(o3, pan3) + Pan2.ar(o4, pan4);

            // --- 5. FILTROS (Dual DJ Topology - Avant Math) ---
            apply_dj_filter = { |in, tone, res, drive, type|
                var ctrl_lp = (tone + 1).clip(0, 1); 
                var ctrl_hp = tone.max(0);
                // Mapeo Logarítmico de Frecuencia
                ...
                // Topología Variable (Moog vs SVF)
                ...
                // High Pass en Serie (Siempre SVF para limpieza)
                var res_sig = RHPF.ar(out_lp, freq_hp, 1.0 - (res * 0.5));
                res_sig;
            };

            // Aplicar Filtros con Modulación
            ...
            // --- 7. SPACE (Tape Echo & Reverb) ---
            delay_in = sig_pre + dirt_sig; 
            local_fb = LocalIn.ar(2);
            local_fb = LeakDC.ar(local_fb);
            local_fb = Limiter.ar(local_fb, 0.95); // Protección Feedback
            
            // Tape Physics
            local_fb = BPeakEQ.ar(local_fb, 100, 1.0, 2.0); // Head Bump
            local_fb = LPF.ar(local_fb, 8000); // Cinta vieja
            local_fb = (local_fb * (1 + (delay_fb * 0.5))).tanh; // Sat Asimétrica
            
            // Erosion (Dropouts)
            local_fb = local_fb * (1.0 - (Decay.kr(Dust.kr(tape_erosion * 10), 0.1) * tape_erosion));

            // Delay Lines con Modulación y Spread
            delay_proc = [
