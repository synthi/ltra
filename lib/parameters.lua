-- code/ltra/lib/parameters.lua | v0.6.1
-- LTRA: Definición de Parámetros (Cobertura Total)

local Params = {}
local Bridge = require 'ltra/lib/engine_bridge'
local Consts = require 'ltra/lib/consts'
local Globals -- Inyección tardía para acceder a tablas si hace falta

-- Helper para actualizar Globals y Engine a la vez
local function set_voice_param(id, key, val)
    -- Actualizar Global
    if Globals and Globals.voices[id] then Globals.voices[id][key] = val end
    -- Actualizar Engine (si corresponde)
    if key == "shape" then Bridge.set_param("shape"..id, val)
    elseif key == "pan" then Bridge.set_param("pan"..id, val)
    elseif key == "vol" then Bridge.set_param("vol"..id, val)
    -- Tune y Arp se gestionan en lógica Lua, no directo al engine param simple
    end
end

function Params.init(g_ref)
    Globals = g_ref
    params:add_separator("LTRA v0.6.1")
    
    -- --- GRUPO: GLOBAL / SCALES ---
    params:add_group("GLOBAL", 4)
    params:add_control("output_level", "Master Vol", controlspec.new(0,1,"lin",0.01,1))
    params:set_action("output_level", function(x) _norns.audio.level_dac(x) end) -- Norns native volume
    
    params:add_number("scale_idx", "Scale", 1, 30, 1)
    params:set_action("scale_idx", function(x) 
        if Globals then Globals.scale.current_idx = x end 
    end)
    
    params:add_number("scale_root", "Root Note", 1, 12, 1)
    params:set_action("scale_root", function(x) 
        if Globals then Globals.scale.root_note = x end 
    end)
    ...
    -- --- GRUPO: VOICES (x4) ---
    params:add_group("VOICES", 24) -- 6 params por voz
    for i=1,4 do
        params:add_separator("Voice "..i)
        
        params:add_control("osc"..i.."_vol", "Vol", controlspec.new(0,1,"lin",0.01,0))
        params:set_action("osc"..i.."_vol", function(x) set_voice_param(i, "vol", x) end)
        
        params:add_control("osc"..i.."_pan", "Pan", controlspec.new(-1,1,"lin",0.01,0))
        params:set_action("osc"..i.."_pan", function(x) set_voice_param(i, "pan", x) end)
        
        params:add_control("osc"..i.."_shape", "Shape", controlspec.new(0,4,"lin",0.01,0))
        params:set_action("osc"..i.."_shape", function(x) set_voice_param(i, "shape", x) end)
        ...
        params:set_action("osc"..i.."_route", function(x) 
            if Globals then Globals.voices[i].to_looper = (x==1) end 
        end)
    end
    ...
    params:add_binary("filt_type", "Type (0=SVF)", "toggle", 0)
    ...
    params:add_control("lfo1_shape", "LFO1 Shape", controlspec.new(0,1,"lin",0.01,0)) -- Morph
    ...
    for i=1,3 do
        ...
        params:set_action("loop"..i.."_vol", function(x) 
            if Globals then Globals.tracks[i].vol = x end 
            -- Softcut level update se hace en loopers.lua refresh, o aqui?
            -- Idealmente aqui llamamos a Loopers.update_levels(i)
        end)
        
        params:add_control("loop"..i.."_speed", "Speed", controlspec.new(-2,2,"lin",0.01,1))
        params:set_action("loop"..i.."_speed", function(x) 
            if Globals then Globals.tracks[i].speed = x end 
            -- Softcut rate update
        end)
        
        params:add_control("loop"..i.."_pan", "Pan", controlspec.new(-1,1,"lin",0.01,0))
        params:set_action("loop"..i.."_pan", function(x) 
            if Globals then Globals.tracks[i].pan = x end 
        end)
        
        params:add_control("loop"..i.."_feedback", "Feedback", controlspec.new(0,1,"lin",0.01,1))
        params:set_action("loop"..i.."_feedback", function(x) 
            if Globals then Globals.tracks[i].feedback = x end 
        end)
        
        params:add_control("loop"..i.."_send", "Send Space", controlspec.new(0,1,"lin",0.01,0))
        params:set_action("loop"..i.."_send", function(x) 
            if Globals then Globals.tracks[i].send_space = x end 
        end)
        
        params:add_binary("loop"..i.."_pre", "Pre/Post", "toggle", 0)
        params:set_action("loop"..i.."_pre", function(x) 
            if Globals then Globals.tracks[i].pre_fx = (x==1) end 
        end)
    end
